...
@Tag(name = "FreeTierSetting (Admin)", description = "FREE/PRO 플랜 월별 생성 횟수 한도 조회/조정 — Admin 전용 [01] 13번, [03] §2-1")
...
public class FreeTierSettingAdminController {
	...
	@Operation(summary = "FREE/PRO 플랜 월별 생성 횟수 한도 조회")
	...
	@Operation(
			summary = "FREE/PRO 플랜 월별 생성 횟수 한도 변경",
	...
	public ResponseEntity<FreeTierLimitResponse> update(@Valid @RequestBody UpdateFreeTierLimitRequest request) {
		return ResponseEntity.ok(FreeTierLimitResponse.from(
				freeTierSettingService.updateLimits(request.freeMonthlyLimit(), request.proMonthlyLimit())));
	}
}

...
public record FreeTierLimitResponse(
		@Schema(description = "FREE 플랜 월별 생성 횟수 한도") int freeMonthlyLimit,
		@Schema(description = "PRO 플랜 월별 생성 횟수 한도") int proMonthlyLimit,
		@Schema(description = "마지막 수정 시각") OffsetDateTime updatedAt) {

	public static FreeTierLimitResponse from(FreeTierSetting setting) {
		return new FreeTierLimitResponse(setting.getFreeMonthlyLimit(), setting.getProMonthlyLimit(), setting.getUpdatedAt());
	}
}

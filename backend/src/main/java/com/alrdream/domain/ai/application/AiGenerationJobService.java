 ...
 * Job 생명주기와 Virtual Thread 비동기 실행만 책임진다.
 */
@Service
public class AiGenerationJobService {
	...
	public AiGenerationJobService(AiGenerationJobRepository aiGenerationJobRepository) {
		this.aiGenerationJobRepository = aiGenerationJobRepository;
	}

	/**
	 * Job을 PENDING 상태로 즉시 생성해 반환하고, {@code task}는 현재 트랜잭션이 커밋된 뒤에야 Virtual Thread에서
	 * 실행한다 — 커밋 전에 비동기 스레드가 먼저 Job을 조회하면 아직 보이지 않는 레코드를 찾게 되는 문제를 막는다.
	 */
	@Transactional

...
public class DesignVersionController {
	...
	@Operation(summary = "설계 버전 목록 조회", description = "삭제되지 않은 버전을 페이지 단위로 최신순 조회한다 (content 미포함).")
	@ApiResponse(responseCode = "200", description = "조회 성공")
	@GetMapping
	public ResponseEntity<PagedModel<DesignVersionSummary>> list(
			@AuthenticationPrincipal MemberPrincipal principal,
			@PathVariable UUID workspaceId,
			@PathVariable UUID planningVersionId,
			@PathVariable UUID analysisVersionId,
		...
		Page<DesignVersionSummary> page = designVersionService
				.list(workspaceId, planningVersionId, analysisVersionId, principal.memberId(), pageable)

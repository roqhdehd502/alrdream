...
- 1시간 무 트래픽 시 **스케일-투-제로**되며 비활성화 불가(유료 플랜에서만 조정 가능) → 유휴 후 첫 요청은 콜드 스타트 지연 발생
- PortOne 웹훅이 콜드 스타트 중 도달하면 응답이 지연될 수 있으나, 포트원이 실패 시 최대 5회 지수 백오프로 재전송하므로 데이터 유실 가능성은 낮음
- 0.1 vCPU / 512MB 스펙은 Spring Boot 구동에 여유가 크지 않으므로 JVM 힙 튜닝(`-Xmx256m` 등)이 필요하며, 추후 콜드 스타트/메모리 개선이 필요하면 GraalVM Native Image 전환을 검토한다
- 트래픽이 늘어나면 스케일-투-제로를 끌 수 있는 유료 플랜으로 전환

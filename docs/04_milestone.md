...
- [x] README.md에 누락사항 확인 후 업데이트
- [x] docs 디렉토리에 구현한 스키마 관련 md 확장자 문서 작성
- [x] docs 디렉토리에 구현한 사항 PPT 발표용으로 정리하여 md 확장자 문서 작성
- [x] 테스트 계정을 통해 실제 사용 동작 예시가 담겨진 사진 캡쳐해서 이미지도 저장할 것

## 작업 내용

- **README.md**: `docs/` 목록에 누락돼 있던 05(색상/폰트)와 신설되는 06/07 문서 링크 추가. 세 앱 모두에
  단위 테스트가 생긴 것(직전 세션)이 반영돼 있지 않아 [테스트](../README.md#테스트) 섹션을 새로 추가하고,
  "Docker 필수" 안내를 "Testcontainers 통합 테스트에만 필요, 순수 단위 테스트는 불필요"로 정정.
- **[06_schema.md](./06_schema.md) 신설**: `database/migarations/`(V1~V10) 전체를 순서대로 적용한
  최종 스키마를 테이블 단위로 정리한 기술 레퍼런스. 03_design.md §5(설계 배경 중심 서술)와 성격이 달라
  중복 대신 상호 참조하도록 함 — ERD(Mermaid), 테이블별 컬럼/제약/추가 버전, 암호화 대상 컬럼 목록, RLS
  정책, 마이그레이션 히스토리 표로 구성.
- **[07_presentation.md](./07_presentation.md) 신설**: PPT 제작용 발표 자료. 프로젝트 소개 → 실제
  스크린샷으로 보는 사용 흐름 → 시스템 구성 → 개발 과정(Phase 요약) → 품질 관리 → 남은 작업 순.
- **스크린샷 캡처**: 개발 서버(`:8080`/`:8081`)를 띄우고 신규 테스트 계정으로 회원가입부터 실제 AI 기획안
  생성까지 전 과정을 라이브로 수행하며 9장을 캡처했다(`docs/images/`) — 합성이 아니라 실제 동작 결과다.
  회원가입 → 이름 설정 → 워크스페이스 생성 → 기획 설문(장문형/척도/단일선택/다중선택 전 문항 타입 포함,
  "잘 모르겠어요" 응답 포함) → 생성 중 화면 → **실제 Claude API로 생성된 기획안 전문**(반려동물 위탁
  서비스 아이디어, 10개 섹션 전부 채워짐) → 워크스페이스 목록(다크 모드) → 구독 비교 화면 → 마이페이지.
  분석 단계 생성은 Free 플랜의 월 1회 한도(이미 기획 생성에 소진)에 막혀 시도하지 않았다 — 오히려 한도
  초과 시 에러 메시지가 깔끔하게 뜨는 것도 함께 확인했다.
- **Admin 화면 스크린샷은 스코프 제외**: 시드된 `admin@alrdream.test` 계정의 실제 비밀번호를 몰라(저장소에
  의도적으로 없음 — `database/seed-test-accounts.sql` 주석 참고) 로그인할 수 없고, DB에 직접 UPDATE로
  새 관리자를 만드는 것은 이번 프로젝트의 안전 정책(운영 DB 직접 조작 금지 원칙을 로컬 개발에도 동일하게
  적용)상 피했다. 사용자에게 확인한 결과 Frontend(사용자 앱) 위주로만 캡처하기로 결정.
- **테스트 계정 정리**: 캡처에 사용한 계정은 작업 종료 후 2단계 확인 탈퇴 절차를 그대로 밟아 정리했고,
  두 서버 모두 종료했다.
